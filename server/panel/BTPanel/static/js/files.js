...
function get_path_size(path) {
    ...
    $.post('/files?action=get_path_size', { path: path }, function (rdata) {
        layer.close(loadT);
        var myclass = '.' + rdata.path.replace(/\//g, '').replace(/\+/g, '-').replace(/\./g, '-');
        console.log(myclass)
		...
		for (var i = 0; i < rdata.DIR.length; i++) {
			...
			if(getCookie("rank") == "a"){
					...
					Body += "<tr class='folderBoxTr' data-path='" + rdata.PATH + "/" + fmp[0] + "' filetype='dir'>\
						...
						<td><a class='btlink "+ (rdata.PATH + '/' + fmp[0]).replace(/\//g,'').replace(/\+/,'-').replace(/\./g,'-') + "' onclick=\"get_path_size('" + rdata.PATH + "/" + fmp[0]+"')\">点击计算</a></td>\
