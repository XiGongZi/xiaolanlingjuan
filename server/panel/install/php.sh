...
Install_PHP_52()
{
    ...
    ./buildconf --force
	if [ "${apacheVersion}" != '2.2' ];then
		./configure --prefix=${php_setup_path} --with-config-file-path=${php_setup_path}/etc --with-mysql=${mysql_dir} --with-pdo-mysql=${mysql_dir} --with-mysqli=${mysql_config} --with-iconv-dir --with-freetype-dir=/usr/local/freetype --with-jpeg-dir --with-png-dir --with-zlib --with-libxml-dir=/usr --enable-xml --enable-discard-path --enable-magic-quotes --enable-safe-mode --enable-bcmath --enable-shmop --enable-sysvsem --enable-inline-optimization --with-curl=/usr/local/curl --enable-mbregex --enable-fastcgi --enable-fpm --enable-force-cgi-redirect --enable-mbstring --with-mcrypt --enable-ftp --with-gd --enable-gd-native-ttf --${withOpenssl} --with-mhash --enable-pcntl --enable-sockets --with-xmlrpc --enable-zip --enable-soap --with-gettext --with-mime-magic --with-iconv=/usr/local/libiconv
	else
		./configure --prefix=${php_setup_path} --with-config-file-path=${php_setup_path}/etc --with-apxs2=$Root_Path/server/apache/bin/apxs --with-mysql=${mysql_dir} --with-pdo-mysql=${mysql_dir} --with-mysqli=$Root_Path/server/mysql/bin/mysql_config --with-iconv-dir --with-freetype-dir=/usr/local/freetype --with-jpeg-dir --with-png-dir --with-zlib --with-libxml-dir=/usr --enable-xml --enable-discard-path --enable-magic-quotes --enable-safe-mode --enable-bcmath --enable-shmop --enable-sysvsem --enable-inline-optimization --with-curl=/usr/local/curl --enable-mbregex --enable-mbstring --with-mcrypt --enable-ftp --with-gd --enable-gd-native-ttf --${withOpenssl} --with-mhash --enable-pcntl --enable-sockets --with-xmlrpc --enable-zip --enable-soap --with-gettext --with-mime-magic
	fi
...
EOF
	...
	if [ "${apacheVersion}" != '2.2' ];then
		...
		update-rc.d php-fpm-52 defaults
...
Set_PHP_FPM_Opt
	...
	sed -i "s/ php-fpm/ php-fpm-53/g" /etc/init.d/php-fpm-53
...
EOF
	...
	sed -i "s/ php-fpm/ php-fpm-54/g" /etc/init.d/php-fpm-54
...
Install_PHP_55()
{
	...
	mv php-${php_55} src
	cd src
	./configure --prefix=${php_setup_path} --with-config-file-path=${php_setup_path}/etc --enable-fpm --with-fpm-user=www --with-fpm-group=www --with-mysql=mysqlnd --with-mysqli=mysqlnd --with-pdo-mysql=mysqlnd --with-iconv-dir --with-freetype-dir=/usr/local/freetype --with-jpeg-dir --with-png-dir --with-zlib --with-libxml-dir=/usr --enable-xml --disable-rpath --enable-bcmath --enable-shmop --enable-sysvsem --enable-inline-optimization --with-curl=/usr/local/curl --enable-mbregex --enable-mbstring --with-mcrypt --enable-ftp --with-gd --enable-gd-native-ttf --${withOpenssl} --with-mhash --enable-pcntl --enable-sockets --with-xmlrpc --enable-zip --enable-soap --with-gettext --disable-fileinfo --enable-opcache --enable-intl
	make ZEND_EXTRA_LIBS='-liconv' -j${cpuCore}
    make install
...
Set_PHP_FPM_Opt
	...
	sed -i "s/ php-fpm/ php-fpm-55/g" /etc/init.d/php-fpm-55
...
Install_PHP_56()
{
	...
	if [ ! -f '/etc/ca-bundle.crt' ];then
		wget -O /etc/ca-bundle.crt https://curl.haxx.se/ca/cacert.pem -T 5
	fi
    ...
    sed -i 's/;cgi.fix_pathinfo=.*/cgi.fix_pathinfo=1/g' ${php_setup_path}/etc/php.ini
    sed -i 's/max_execution_time =.*/max_execution_time = 300/g' ${php_setup_path}/etc/php.ini
	sed -i 's/;openssl.cafile=/openssl.cafile=\/etc\/ca-bundle.crt/' ${php_setup_path}/etc/php.ini
	sed -i 's/;curl.cainfo =/curl.cainfo = \/etc\/ca-bundle.crt/' ${php_setup_path}/etc/php.ini
    sed -i 's/disable_functions =.*/disable_functions = passthru,exec,system,chroot,chgrp,chown,shell_exec,popen,proc_open,ini_alter,ini_restore,dl,openlog,syslog,readlink,symlink,popepassthru/g' ${php_setup_path}/etc/php.ini
    sed -i 's/display_errors = Off/display_errors = On/g' ${php_setup_path}/etc/php.ini
    Pear_Pecl_Set
    Install_Composer
...
Set_PHP_FPM_Opt
	...
	sed -i "s/ php-fpm/ php-fpm-56/g" /etc/init.d/php-fpm-56
	update-rc.d php-fpm-56 defaults
...
Set_PHP_FPM_Opt
	...
	sed -i "s/ php-fpm/ php-fpm-70/g" /etc/init.d/php-fpm-70
	update-rc.d php-fpm-70 defaults
...
Set_PHP_FPM_Opt
	...
	sed -i "s/ php-fpm/ php-fpm-71/g" /etc/init.d/php-fpm-71
	update-rc.d php-fpm-71 defaults
...
Set_PHP_FPM_Opt
	...
	sed -i "s/ php-fpm/ php-fpm-72/g" /etc/init.d/php-fpm-72
	update-rc.d php-fpm-72 defaults
...
Install_PHP_73()
{
	...
	if [ ! -f "/usr/local/curl2/bin/curl" ];then
		...
		if [ -d "/usr/local/openssl"  ]; then
			...
			./configure --prefix=/usr/local/curl2 --enable-ares --without-nss --with-ssl
		fi
		make -j${cpuCore}
		make install
		cd ..
	...
	sed -i 's/;openssl.cafile=/openssl.cafile=\/etc\/ssl\/certs\/ca-certificates.crt/' ${php_setup_path}/etc/php.ini
	sed -i 's/;curl.cainfo =/curl.cainfo = \/etc\/ssl\/certs\/ca-certificates.crt/' ${php_setup_path}/etc/php.ini
    sed -i 's/disable_functions =.*/disable_functions = passthru,exec,system,chroot,chgrp,chown,shell_exec,popen,proc_open,ini_alter,ini_restore,dl,openlog,syslog,readlink,symlink,popepassthru/g' ${php_setup_path}/etc/php.ini
...
Set_PHP_FPM_Opt
    ...
    update-rc.d php-fpm-73 defaults
...
Uninstall_PHP()
{
	php_version=${1/./}
	/etc/init.d/php-fpm-$php_version stop
	update-rc.d -f php-fpm-$php_version remove
...
if [[ "${Openssl_Version}" =~ "1.1." ]]; then
	Install_OpenSSL
	withOpenssl="with-openssl=/usr/local/openssl"
else
	withOpenssl="with-openssl"
fi
