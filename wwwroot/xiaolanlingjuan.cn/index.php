 ...
 *
 * */
$appId = '1229358';  // 站点的APPID （请勿修改和泄漏）
$appKey = 'E426FDE5F914C28DE6155B2285BF777F';// 站点的APP KEY（请勿修改和泄漏）
$proxyVersion = 100;
$autoCleanCache = 200;
...
if ($requestMethod == 'GET' && $httpHelper->httpCode == 200 && !empty($html) && !$test_env) {
    require_once 'hunki/hunki.php';
    $html = hunkiHtml($html);
//    $cache->Set($key, $html, 60);
    //这里加载自定义css和javascript脚本
    $loder = new Loader();
    $loder->setHtml($html);
    $html = $loder->getContent();
}
if (!empty($html)) {
    echo $html;
} else {
