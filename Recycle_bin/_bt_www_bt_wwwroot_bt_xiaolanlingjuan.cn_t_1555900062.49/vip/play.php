...
/* 修复 动漫 不显示集数 by fy, 20181023 */
$tvzz = "#<div class=\"m-series-content\\sjs-tab-content\" data-default=\".*\">[\\s\\S]+?<a\\s*data-num=\"(.*?)\"\\s*data-daochu=\"to=(.*?)\"\\shref=\"(.*?)\">[\\s\\S]+?</div>#";
$tvzz1 = "#<a\\s*data-num=\"(.*?)\"\\s*data-daochu=\"to=(.*?)\"\\shref=\"(.*?)\">#";

$bflist = "#<a data-daochu=\"to=(.*?)\" class=\"btn js-site ea-site ea-site-(.*?)\" href=\"(.*?)\">(.*?)</a>#";
$jianjie = "#<p class=\"item-desc\">(.*?)<#";
$biaoti = "#<h1>(.*?)</h1>#";
...
<?php
if ($url_array[1] == "m") {
?><iframe id="video" allowFullScreen=ture src="<?php echo $aik["jiekou1"];?><?php echo $mvsrc[0];?>" style="width:100%;border:none"></iframe><?php
} else {
