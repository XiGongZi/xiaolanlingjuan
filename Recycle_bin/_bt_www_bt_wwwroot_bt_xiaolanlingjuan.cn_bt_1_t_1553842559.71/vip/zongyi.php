...
foreach ($zycat as $kcat => $vcat) {
	$flzyname = $zyname[$kcat];
	$flid = "list.php?cat=" . $vcat . "&pageno=1";
?><a href='?m=<?php echo $flid;?>' target='_self'><?php echo $flzyname;?></a><?php
}
?></dd>
</dl>
</div>
...
if (empty($_GET["pageno"])) {
	$html = "http://www.360kan.com/zongyi/list.php?cat=all&pageno=1";
	$cur_page = "1";
} else {
	$html = "http://www.360kan.com/zongyi/list.php?cat=all" . "&pageno=" . $_GET["pageno"];
