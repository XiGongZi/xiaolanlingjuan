...
class Loader
{
    ...
    public function loadCss()
    {
        ...
        $hunkiCss = __DIR__ . '/hunki/loader/common.css';
        if (file_exists($hunkiCss)) {
            $time = time();
            $html = str_ireplace('</head>', "<link rel='stylesheet' href='hunki/loader/common.css?v={$time}' /></head>", $this->html);
            $this->setHtml($html);
        }
        //替换header
        return $this;
